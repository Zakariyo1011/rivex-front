...
<template>
  <div ref="root" class="relative">
    <button
      class="w-10 h-10 rounded-full bg-white border border-border flex items-center justify-center relative shrink-0"
      @click="toggle"
    >
      🔔
      <span
    ...
    <div
      v-if="open"
      class="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-white rounded-2xl shadow-lg border border-border z-50"
    >
      <div class="flex items-center justify-between px-4 py-3 border-b border-border">
        <span class="font-semibold text-ink">Bildirishnomalar</span>
        <button v-if="store.unreadCount > 0" class="text-xs text-primary-600 font-medium" @click="store.markAllRead">
          Barchasini o'qish
        </button>
      </div>

      <div v-if="store.notifications.length === 0" class="text-sm text-ink-faint text-center py-8">
